...
const mySubmit = (event) =>{
  ...
  let wordBox = document.getElementById('user-input');
  let word = wordBox.value;
  let rList = document.getElementById('results');
  // after retrieving the word input value, print it to the console for check
  console.log(word);
  
  //run the pigLatin function and store result in a variable
  let newWord = pigLatin(word);
  console.log(newWord);
  
  //create a new LI called newLI
  let newLI = document.createElement('li');
  
  //set newLI inner text to the new word
  newLI.innerText = newWord;
  //add the result word to a list
  rList.appendChild(newLI);
  
  //reset the box value to blank
  wordBox.value = '';
}
